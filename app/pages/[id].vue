...
// Check if connected wallet matches the node address (for showing earnings)
const isOwner = computed(() => {
  return activeAddress.value?.toLowerCase() === nodeAddress.value?.toLowerCase();
});

// Remove wallet connection prompt - users can view any node, earnings only if owner
...
// API call for earnings history (only if owner)
const selectedPeriod = ref<'daily' | '3' | '3000'>('3000');
const earningsHistoryEndpoint = computed(() => {
  if (!isOwner.value || !nodeAddress.value) return undefined;
  ...
  return `/api/stats/earning-history?address=${nodeAddress.value}&start_date=${formatDate(finalStartDate)}&group_by=${groupBy}`;
});
...
const uptimeEarningsEndpoint = computed(() => {
  ...
  return `/api/nodes/heartbeats/uptime/${nodeAddress.value}?startDate=${startDateString}&endDate=${endDateString}`;
});
...
// Add watcher for isOwner and selectedPeriod to trigger earnings fetch
watch([isOwner, selectedPeriod], ([owner, newPeriod]) => {
  if (owner && earningsHistoryEndpoint.value) {
