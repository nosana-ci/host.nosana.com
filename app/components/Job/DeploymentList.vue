  ...
  <table class="table is-fullwidth is-striped is-hoverable mb-0" :class="{ 'is-small': small }">
    ...
    <tbody>
      ...
      <tr v-for="job in jobs" v-else :key="job.address" class="clickable-row" @click="navigateToJob(job.address)">
        <td>
          <a :href="`https://explore.nosana.com/jobs/${job.address}`" target="_blank" rel="noopener noreferrer" class="address is-family-monospace" @click.stop>
            {{ job.address }}
          </a>
        </td>
        ...
        <td v-if="!small" class="is-hidden-touch">
          <span class="clickable-row-cell-content">
            <a
              ...
              :href="`https://explore.nosana.com/markets/${job.market.toString()}`"
              target="_blank"
...
import SecondsFormatter from "~/components/UI/SecondsFormatter.vue";
...
const navigateToJob = (jobAddress: string) => {
  window.open(`https://explore.nosana.com/jobs/${jobAddress}`, '_blank', 'noopener,noreferrer');
};
